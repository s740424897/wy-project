    ...
    <ul class="popular_goods" v-if="homeData.popularItemList">
      <li v-for="(item,index) in homeData.popularItemList" :key="index">
        ...
        <div class="popular_info">
          <span class="popular_name">{{index===0?item.name:(item.name.length > 14?(item.name.slice(0,14)+'...'):item.name)}}</span>
          <span class="popular_desc" v-if="index===0">{{item.simpleDesc}}</span>
